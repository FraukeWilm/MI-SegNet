...
from MI_SegNet import Mine_Conv, Recon_encoder_LM, Recon_decoder_LM
...
class MIUnetModule(pl.LightningModule):
    def __init__(self, cfg, device, **kwargs):
        ...
        self.mine = Mine_Conv(in_channels_x=512, in_channels_y=16 * 16, inter_channels=64, last_pooling_size=int(np.power(2, cfg.data.patch_size//256))).to(device)
        self.rec_encoder = Recon_encoder_LM(in_channels=cfg.model.input_channel, init_features=16).to(device)
        self.rec_decoder = Recon_decoder_LM(in_channels_a=512, in_channels_d=16 * 16,
    ...
    def update_Rec_Adv(self, inputs_1, inputs_2, inputs_12, train):
        z_a = self.seg_encoder(inputs_2)[-1]
